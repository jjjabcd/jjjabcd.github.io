...
However, we consider it good style to mention author last names if you discuss something at length and it fits into the flow well?됤붴뎥he authors are human and it?셲 nice for them to have the community associate them with their work.
...
And here?셲 how it will be rendered:
...
This theme also supports integrating [Diff2Html](https://github.com/rtfpessoa/diff2html), a tool that beautifully renders code differences (diffs) directly in markdown. Diff2Html is ideal for showcasing code changes, allowing you to clearly present additions, deletions, and modifications. It?셲 perfect for code reviews, documentation, and tutorials where step-by-step code changes need to be highlighted?봸ou can even introduce changes across multiple files at once.
...
Here?셲 how it will look when rendered with Diff2Html:
...
[Chart.js](https://www.chartjs.org/) is a versatile JavaScript library for creating responsive and interactive charts. Supporting multiple chart types like bar, line, pie, and radar, it?셲 an ideal tool for visualizing data directly in webpages.

Here?셲 an example of a JSON-style configuration that creates a bar chart in Chart.js:
...
The rendered output shows a clean and simple bar chart with a hover effect竊?
```vega_lite
{
  "$schema": "https://vega.github.io/schema/vega/v5.json",
  "width": 400,
...
[TikZ](https://tikz.net/) is a powerful LaTeX-based drawing tool powered by [TikZJax](https://tikzjax.com/). You can easily port TikZ drawings from papers, posters, and notes. For example, we can use the following code to illustrate Euler?셲 formula $ e^{i \theta} = \cos \theta + i \sin \theta $:
...
The rendered output is shown below, displayed as a vector graphic竊?
<script type="text/tikz">
\begin{document}
  \begin{tikzpicture}
      \filldraw[fill=cyan!10, draw=blue, thick] (0,0) circle (2cm);
...
The rendered output is shown below竊?
```typograms
             ___________________
            /                  /|
           /__________________/ |
...
Occasionally you?셪l want to use the full browser width.
...
<aside>
  ...
  <p>
    F.J. Cole, ?쏷he History of Albrecht D체rer?셲 Rhinoceros in Zoological Literature,??Science, Medicine, and History: Essays on the Evolution of
    Scientific Thought and Medical Practice (London, 1953), ed. E. Ashworth Underwood, 337-356. From page 71 of Edward Tufte?셲 Visual Explanations.
  </p>
</aside>
```
...
<aside>
  {% include figure.liquid loading="eager" path="assets/img/rhino.png" class="img-fluid rounded z-depth-1" zoomable=true %}
  <p>F.J. Cole, ?쏷he History of Albrecht D체rer?셲 Rhinoceros in Zoological Literature,??Science, Medicine, and History: Essays on the Evolution of Scientific Thought and Medical Practice (London, 1953), ed. E. Ashworth Underwood, 337-356. From page 71 of Edward Tufte?셲 Visual Explanations.</p>
</aside>

Sidenotes can also contain equations and links:

In physics, mass?밻nergy equivalence is the relationship between mass and energy in a system's rest frame. The two differ only by a multiplicative constant and the units of measurement.
...
2. Another item
   ?끸땯\* Unordered sub-list.
3. Actual numbers don't matter, just that it's a number
   ?끸땯1. Ordered sub-list
4. And another item.

?끸땯?꿠ou can have properly indented paragraphs within list items. Notice the blank line above, and the leading spaces (at least one, but we'll use three here to also align the raw Markdown).

?끸땯?꿚o have a line break without a paragraph, you will need to use two trailing spaces.?끸땯
?끸땯?꿓ote that this line is separate, but within the same paragraph.?끸땯
?끸땯??This is contrary to the typical GFM line break behaviour, where trailing spaces are not required.)
