...
$$
\mathcal{L}(\tau, \omega; \theta) = \left( \log \frac{Z_\theta(\omega) \prod_{s \to s' \in \tau} P_F(s' \mid s, \omega; \theta)}{R(x \mid \omega) \prod_{s \to s' \in \tau} P_B(s \mid s', \omega; \theta)} \right)^2
$$
...
$$
R(x \mid \omega) = \prod_{i=1}^{d} R_i(x)^{\omega_i}
$$
