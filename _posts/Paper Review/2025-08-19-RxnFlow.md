...
$$
P_F(s'|s) = \frac{F(s \to s')}{F(s)}, \quad P_B(s|s') = \frac{F(s \to s')}{F(s')}
$$
...
Subsampling policy $\mathcal{P}(\mathcal{A})$는 initial state에서는 uniform sampling을, 이후 state에서는 importance sampling을 수행한다. 희귀 action을 보존하기 위해 `Stop`과 `ReactUni`는 모두 사용하고, `ReactBi`의 reactant block만 uniform subset $\mathcal{B}_r^* \subseteq \mathcal{B}_r$로 sampling한다고 이야기한다.

$$
\mathcal{A}^*(s) = \begin{cases} \mathcal{B}^* & \text{if } s = s_0 \\ \{\text{Stop}\} \cup \mathcal{R}_1 \cup \{(r, b) | r \in \mathcal{R}_2, b \in \mathcal{B}_r^*\} & \text{otherwise} \end{cases}
$$

여기서 $\mathcal{B}^*$는 initial state에서 sampling된 building block subset, $\mathcal{B}_r^*$는 bi-molecular reaction $r$에 대해 sampling된 reactant block subset을 의미한다.

Importance sampling 보정을 위해 edge flow에는 subsampling ratio에 따른 weight $w_a$를 곱한다.

$$
w_a = \begin{cases} |\mathcal{B}|/|\mathcal{B}^*| & \text{if } a \in \mathcal{B} \\ |\mathcal{B}_r|/|\mathcal{B}_r^*| & \text{if } a \text{ is } (r, b), r \in \mathcal{R}_2 \\ 1 & \text{if } a \text{ is Stop or } a \in \mathcal{R}_1 \end{cases}
$$
...
여기서 $\hat{F}_\theta(s; \mathcal{A}^*)$는 partial action space $\mathcal{A}^*$로부터 추정된 state flow를 의미한다. 추정된 forward policy는 $\hat{P}_F(s'|s; \mathcal{A}^*; \theta) = F_\theta(s \to s')/\hat{F}_\theta(s; \mathcal{A}^*)$로 정의된다고 설명한다.

논문에서는 이 추정이 unbiased estimator임을 부록 A에서 증명하고 있으며, 계산 복잡도를 $O(|\mathcal{B}||\mathcal{R}_2|)$에서 $O(|\mathcal{B}^*||\mathcal{R}_2|)$로 감소시킬 수 있다고 주장한다.
...
$$
P_F(T(s, (r, b))|s; \theta) = \frac{F_\theta(s, (r, b))}{\sum_{r' \in \mathcal{R}_1 \cup \{\text{Stop}\}} F_\theta(s, r') + \sum_{r' \in \mathcal{R}_2} \sum_{b' \in \mathcal{B}_{r'}} F_\theta(s, (r', b'))}
$$
...
$$
\hat{\mathcal{L}}_{\text{TB}}(\tau) = \left( \log \frac{Z_\theta \prod_{t=1}^{n} \hat{P}_F(s_t|s_{t-1}; \mathcal{A}_{t-1}^*; \theta)}{R(x) \prod_{t=1}^{n} P_B(s_{t-1}|s_t)} \right)^2
$$
...
- Line 3에서는 각 transition step별로 subsampling policy $\mathcal{P}(\mathcal{A})$를 통해 partial action space $\mathcal{A}_t^*$를 sampling한다고 이야기한다.
