...
위 수식에서 $Z_\theta$는 partition function의 학습 가능한 estimate를 의미하고, $P_F(s_t | s_{t-1})$는 state $s_{t-1}$에서 $s_t$로의 forward transition probability를 의미한다. $P_B(s_{t-1} | s_t)$는 backward transition probability를 나타낸다.
...
$$
p(m_i | \varnothing) = \sigma^{|M|}(s)_i, \quad s_i = \mathrm{MLP}_M(f(\varnothing))_i
$$

위 수식에서 $\mathrm{MLP}_M : \mathbb{R}^D \to \mathbb{R}^{|M|}$는 multi-layer perceptron을 의미하고, $\sigma^k$는 길이 $k$의 logit vector 위의 softmax function이다. $\varnothing$는 empty graph를 의미한다.

Reaction template을 선택할 때 $i$번째 reaction $r_i$를 고를 확률은 다음과 같이 정의한다.

$$
p(r_i | m) = \sigma^{|R|+1}(s)_i, \quad s_i = \mathrm{MLP}_R(f(m))_i
$$

위 수식에서 $\mathrm{MLP}_R$은 $|R|+1$개의 logit을 출력하는데, 마지막 logit은 stop action에 해당한다. 적용 불가능한 reaction의 score $s_i$는 $-\infty$로 mask한다.

두 번째 reactant $m_i$를 고를 확률은 다음과 같이 정의한다.

$$
p(m_i | m, r) = \sigma^{|M|}(s)_i, \quad s_i = \mathrm{MLP}_M(f(m, r))_i
$$
...
$$
p(m'_i) = \sigma^{|M'|}(s)_i, \quad s_i = \mathrm{MLP}_{M'}(f(m'_i))
$$
...
$$
p((r_i, m'_i, m''_i) | m) = \sigma^{|T|}(s)_i, \quad s_i = \mathrm{MLP}_B(f(m'_i, r_i))
$$
...
$$
p(m_i | m, r) = \sigma^{|M|}(s)_i, \quad s_i = \phi(W f(m, r))^T g(m_i)
$$
