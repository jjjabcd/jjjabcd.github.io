...
위 수식에서 $$A^\pi(s_t,a_t)>0$$이면 현재 state에서 선택한 action이 평균보다 좋은 action이라고 해석할 수 있다. 따라서 Actor는 해당 action of probability를 증가시키는 방향으로 update된다.
